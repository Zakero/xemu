...
static void ui_cb_keymap_load ( const struct menu_st *m, int *query )
{
	if (query) return;
	//hid_keymap_from_config_file(configdb.keymap);
	ERROR_WINDOW("TODO for userdata %d", VOIDPTR_TO_INT(m->user_data));
}
#endif
...
#ifdef HID_KBD_MAP_CFG_SUPPORT
static const struct menu_st menu_keymap[] = {
	{ "Load custom keymap config",	XEMUGUI_MENUID_CALLABLE,	ui_cb_keymap_load, (void*)1 },
	{ "Use the default keymap",	XEMUGUI_MENUID_CALLABLE,	ui_cb_keymap_load, (void*)0 },
	{ NULL }
};
#endif
static const struct menu_st menu_config[] = {
#ifdef HID_KBD_MAP_CFG_SUPPORT
	{ "Keymap",			XEMUGUI_MENUID_SUBMENU,		NULL, menu_keymap },
#endif
