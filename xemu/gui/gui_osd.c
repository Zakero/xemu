...
{
	//DEBUGPRINT("OSD-GUI message via %s(): \"%s\", \"%s\"" NL, __func__, title, msg);
	...
	for (int a = 0; a < nbuttons; a++) {
		...
		sprintf(
			text, "%s-%s-%d-> %s",
			(buttons[a].flags & SDL_MESSAGEBOX_BUTTON_RETURNKEY_DEFAULT) ? "Enter" : "-----",
			(buttons[a].flags & SDL_MESSAGEBOX_BUTTON_ESCAPEKEY_DEFAULT) ? "ESC"   : "---",
			a + 1,
			buttons[a].text
		);
		_osdgui_format_text((const Uint8*)text, 0, y++, 5, 3);
	}
	y++;
	...
	for (int need_rendering = 1;;) {
		...
		for (int a = 0; a < nbuttons; a++) {
			if (
				(ret == OSDGUIKEY_ENTER && (buttons[a].flags & SDL_MESSAGEBOX_BUTTON_RETURNKEY_DEFAULT)) ||
				(ret == OSDGUIKEY_ESC   && (buttons[a].flags & SDL_MESSAGEBOX_BUTTON_ESCAPEKEY_DEFAULT))
			) {
				result = buttons[a].buttonid;
				goto take;
			}
			if (ret >= '1' && ret <= '9' && a == (ret - '1')) {
				result = buttons[a].buttonid;
				goto take;
			}
		}
		if (ret == OSDGUIKEY_QUIT)
			goto take;
	}
take:
	osd_off();
